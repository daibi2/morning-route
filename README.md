...
1. **发布前体检**：确认环境变量已按目标环境配置（`DATABASE_URL`、`JWT_SECRET`、`PORT`、`WEB_ORIGIN`、`NODE_ENV`），生产环境务必替换 `.env.example` 中的默认 `JWT_SECRET`；`.env` 不入库。
2. **依赖与检查**：`npm install` 后依次执行 `npm run lint`、`npm test`，全部通过后再发布；首次运行 E2E 前需先安装浏览器：`npx playwright install --with-deps chromium`。
...
5. **发布后验证**：`GET /api/health` 应返回 200（API 默认监听 `127.0.0.1`，生产需经反向代理暴露，健康检查请从代理或主机内发起）；再打开 Web 首页确认注册/登录、打卡与统计页面正常。
6. **密钥与配置**：生产密钥通过部署平台注入，禁止写入仓库或前端构建产物。
7. **回滚**：应用回滚到上一个已验证版本；数据库如需回滚，按 `prisma/migrations` 的逆序人工编写并执行回退脚本，禁止直接删除或重建数据表。
